...
**问题**：机器人坐下后，如果牌桌上没有其他玩家，会在 20 秒后自动离开。

**代码位置**：`lifecycle_manager.py`
```python
_SITOUT_LEAVE_DELAY = 20  # 秒
```

**建议**：
- 增加等待时间到 60 秒
- 或添加"最少玩家数"判断（至少 2 人以上才开始游戏）

### 5.2 座位 ID 识别错误

**问题**：从 DOM 提取座位 ID 时可能出错，导致 `my_seat_id` 与实际不符。

**代码位置**：`lifecycle_manager.py` - `_find_my_seat()`

**建议**：使用 WebSocket 中的 `userId` 匹配来确认自己的座位。

### 5.3 统计数据不准确

**问题**：有时 `Start Stack` 显示为 0，但实际应该是买入金额。

**代码位置**：`browser_manager.py` - `_accumulate_table_stats()`
...
# 运行特定模块测试
python -m pytest tests/unit/bot/test_lifecycle_manager.py -v
```
...
| 文件 | 说明 |
|------|------|
| `test_browser_manager.py` | 浏览器管理测试 |
| `test_lifecycle_manager.py` | 入座/离场测试 |
| `test_lobby_manager.py` | 大厅管理测试 |
| `test_play_manager.py` | 游戏状态解析测试 |
| `test_table_manager.py` | 牌桌协调测试 |
| `test_task_manager.py` | 任务目标测试 |
| `test_dom_parsing.py` | DOM 解析测试（使用真实 DOM） |
| `test_play_manager_raise.py` | 加注尺度测试 |
| `fixtures/` | 真实 DOM 快照文件 |
